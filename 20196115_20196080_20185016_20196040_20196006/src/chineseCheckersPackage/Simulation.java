...
import chineseCheckersPackage.ChineseCheckersGameGUI.GameState;
...
public class Simulation{
	public static Seed[] crossId = {Seed.CROSS0, Seed.CROSS1, Seed.CROSS2, Seed.CROSS3, Seed.CROSS4, Seed.CROSS5, 
	   ...
	   public Simulation (Seed[][] boardSim) {
		   gameBoard = copyGameBoard(boardSim);
	   }
	   
	   
	   public Node simulateGame(Seed seedSim, Node moveNode, int[] move) {
		   Seed[][] boardSimulation = copyGameBoard(gameBoard);
		   ...
		   if (moveNode == null) {
			   ...
			   moveNode = findChildStates(moveNode, move[0], move[1], move[2], move[3]);
			   moveNode.createChildren(moveNode);
		   }
		   
		   moveNode.setParent(null);

		   Node bestMove = runSimulation(boardSimulation, moveNode);
		   
		   return bestMove;
	   }
	   

	   private Node runSimulation(Seed[][] gaming, Node rootNode) {	   
		   exploreChildStates(rootNode);
		   ...
		   while (System.currentTimeMillis() - start < (timeMaxSeconds*1000)) {
			   ...
			   Node node = selectMostPromisingMove(rootNode);
			   ...
			   while (currentState == GameState.PLAYING) {
				   if (node.getChildStateArray().size() <= 0) {
					   node.createChildren(node);
				   }
				   
				   Node nextMove = selectMostPromisingMove(node);
				   ...
				   currentState = updateGameState(node.getBoard(), node.getPlayer());
		   ...
		   for (int i = 0; i < rootNode.getChildStateArray().size(); i++) {
			   double wins = rootNode.getChildStateArray().get(i).getWins();
			   double plays = rootNode.getChildStateArray().get(i).getPlays();
			   double winRate = wins/plays;

			   wins = rootNode.getChildStateArray().get(best).getWins();
			   plays = rootNode.getChildStateArray().get(best).getPlays();
			   double topRate = wins/plays;
			   System.out.println("Final Nodes: " + rootNode.getChildStateArray().get(i).getPlayer() + " " + 
					   rootNode.getChildStateArray().get(i).getPiece() + " " + rootNode.getChildStateArray().get(i).getWins() + " " +
					   rootNode.getChildStateArray().get(i).getPlays() + " " + winRate + " " + rootNode.getChildStateArray().get(i).getX0() + " " + 
					   rootNode.getChildStateArray().get(i).getY0() + " " + rootNode.getChildStateArray().get(i).getX() + " " + 
					   rootNode.getChildStateArray().get(i).getY());
			   if (winRate > topRate) {
				   best = i;
				   tie.clear();
				   tie.add(best);
		   ...
		   System.out.println("Best Node: " + rootNode.getChildStateArray().get(best).getPlayer() + " " + 
				   rootNode.getChildStateArray().get(best).getPiece() + " " + rootNode.getChildStateArray().get(best).getWins() + " " +
				   rootNode.getChildStateArray().get(best).getPlays() + " " + rootNode.getChildStateArray().get(best).getX0() + " " + 
				   rootNode.getChildStateArray().get(best).getY0() + " " + rootNode.getChildStateArray().get(best).getX() + " " + 
				   rootNode.getChildStateArray().get(best).getY());
		   System.out.println("Root Node: " + rootNode.getPlayer() + " " + rootNode.getWins() + " " + rootNode.getPlays());
		   return rootNode.getChildStateArray().get(best);
	   }

	   private Node selectMostPromisingMove(Node node) {
		   int best = 0;
		   ArrayList<Double> select = new ArrayList<Double>();
		   select.ensureCapacity(node.getChildStateArray().size());
		   for (int i = 0; i < node.getChildStateArray().size(); i++) {
			   if (node.getChildStateArray().get(i).getPlays() <= 0) {
				   select.add(Math.random());
			   } else if (node.getChildStateArray().get(i).getPlays() > 0) {
				   double parentPlays = node.getPlays();
				   double wins = node.getChildStateArray().get(i).getWins();
				   double plays = node.getChildStateArray().get(i).getPlays();
		   ...
		   return node.getChildStateArray().get(best);
	   }
	   ...
	   private void exploreChildStates(Node rootNode) {
		   int count = rootNode.getChildStateArray().size();
		   for (int i = 0; i < count; i++) {
			   Node node = rootNode.getChildStateArray().get(i);
			   /*System.out.println("Promising Node: " + node.getPlayer() + " " + 
					   node.getPiece() + " " + node.getWins() + " " + node.getPlays() + " " + 
					   node.getX0() + " " + node.getY0() + " " + node.getX() + " " + node.getY());*/
	   ...
	   public Node findChildStates(Node ogNode, int x0, int y0, int x, int y) {
		   Node returnNode = null;
		   //System.out.println("Expected: " + x0 + " " + y0 + " " + x + " " + y);
		   for (int i = 0; i < ogNode.getChildStateArray().size(); i++) {
			   ...
			   if (ogNode.getChildStateArray().get(i).getX0() == x0 && ogNode.getChildStateArray().get(i).getY0() == y0 &&
					   ogNode.getChildStateArray().get(i).getX() == x && ogNode.getChildStateArray().get(i).getY() == y) {
				   returnNode = ogNode.getChildStateArray().get(i);
	   ...
	   public Seed[][] copyGameBoard(Seed[][] gamingBoard) {
	   ...
	   public GameState updateGameState(Seed[][] game, Seed theSeed) {
	   ...
	   public ArrayList<Integer> movesSimulation(Seed[][] game, Seed player, int oldX, int oldY, int newX, int newY) {
		   Seed[][] simBoard = copyGameBoard(game);
				   ...
				   for (int i = 0; i < count; i++) {
					   if (movesC[(i*2)] < 17 && movesC[(i*2)] > -1 && movesC[(i*2)+1] > -1 && movesC[(i*2)+1] < 13) {
						   if (checkPresent(crossId, simBoard[movesC[i*2]][movesC[(i*2)+1]]) || checkPresent(noughtId, simBoard[movesC[i*2]][movesC[(i*2)+1]])) {
							   ArrayList<Integer> setMoves = movesSimulation(simBoard, currentPlayer, xM, yM, movesC[(i*2)], movesC[(i*2)+1]);
			   ...
			   for (int i = 0; i < count; i++) {
				   if (movesC[(i*2)] < 17 && movesC[(i*2)] > -1 && movesC[(i*2)+1] > -1 && movesC[(i*2)+1] < 13) {
					   ...
					   if (checkPresent(crossId, gameB[movesC[(i*2)]][movesC[(i*2)+1]]) || checkPresent(noughtId, gameB[movesC[(i*2)]][movesC[(i*2)+1]])) {
						   ArrayList<Integer> setMoves = movesSimulation(gameB, player, x, y, movesC[(i*2)], movesC[(i*2)+1]);
						   for (int j = 0; j < setMoves.size(); j++) {
